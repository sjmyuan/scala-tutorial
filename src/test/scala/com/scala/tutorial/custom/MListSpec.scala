...
import com.scala.tutorial.custom.category.data.::
...
class MListSpec extends FunSpec with Matchers {
  ...
  describe("MList") {
    describe("instance") {
      ...
      it("can be used in constructor match expression") {
      ...
      it("can be used in head :: tail match expression") {
        val result = MList(1, 2) match {
          case head :: tail => head
          case _ => 0
        }
    ...
    describe("::") {
      it("can insert value to the head"){
        1 :: END should be(MList(1))
      }
    }
